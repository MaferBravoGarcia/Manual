...
# Estructura de los Datos {#estructura} 
...
### Ejemplo: Detección de Secuencias Específicas

Supongamos que tenemos un vector de secuencias de ADN y queremos identificar si alguna de las secuencias contiene un motivo específico, como "AGCT".
...
cat("Secuencias con el motivo", motivo, ":", secuencias_con_motivo, "\n")
```
...
- `/R`: Guarda los scripts de R (.R o .Rmd).
- `/data`: Almacena los conjuntos de datos originales (preferentemente en formato de solo lectura como .csv o .xlsx).
- `/doc`: Contiene documentos relacionados, como manuscritos.
- `/figs`: Guarda las figuras generadas por tus scripts.
- `/output`: Almacena archivos de salida procesados, como datos limpios o tablas resumen.
